...
def login_lagi334():
	try:
		...
		print(f'  %s[%sx%s]%s LOGIN GAGAL.....CEK TUMBAL LUU BRE !!%s'%(x,k,x,m,x))
		exit()
def bot():
	try:
...
#-------------[ PENGATURAN-IDZ ]---------------#
def setting():
	cetak(nel('[cyan][white]1.Akun Old\n[cyan][white]2.Akun new[white] '))
	print('')
	hu = input('>> Pilih : ')
	if hu in ['1','01']:
...
def passwrd():
	...
	print(f'>> Mainkan Mode Pesawat Setiap {m}500k{x} Idz\n')
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r𝙩𝙧𝙞𝙨𝙮𝙖 {P}[{b}{loop}{P}/{u}{len(id)}{P}]—{P}[{H}{ok}{P}]—{P}[{k}{cp}{x}]—[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
